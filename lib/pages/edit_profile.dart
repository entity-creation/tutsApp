...
class _EditProfileState extends State<EditProfile> {
  late final Set<CheckBoxState> studies = {};
  final stud = [
    CheckBoxState(title: "Java"),
    CheckBoxState(title: "C++"),
    CheckBoxState(title: "Cloud Computing")
  ];
        ...
        builder: (context, snapshot) {
          if (snapshot.hasData) {
            ...
            return StreamBuilder<Dept>(
                stream: FirebaseApi.getDept(),
                builder: (newcontext, newsnapshot) {
                  if (newsnapshot.hasData) {
                    final list = newsnapshot.data;

                    dept.clear();

                    list!.dept.forEach(
                      (element) => dept.add(element),
                    );

                    return StreamBuilder<Courses>(
                        stream: FirebaseApi.getCourse(),
                        builder: ((acontext, asnapshot) {
                          Set tempcourses = {};
                          if (asnapshot.hasData) {
                            final list = asnapshot.data;
                            print(list);
                            ...
                            list!.courses.forEach(
                              (element) {
                                tempcourses.add(element);
                              },
                            );
                            // List Mycourses = tempcourses as List;

                            tempcourses.forEach(
                              (element) {
                                studies.add(CheckBoxState(title: element));
                              },
                            );
                            final List<CheckBoxState> myStudies =
                                List.unmodifiable(studies);
                            return SafeArea(
                              child: Scaffold(
                                appBar: AppBar(
